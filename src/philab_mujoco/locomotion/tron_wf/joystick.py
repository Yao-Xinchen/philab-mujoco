...
from philab_mujoco.locomotion.tron_wf import base
from philab_mujoco.locomotion.tron_wf import constants as consts


def tron_wf_joystick_config() -> config_dict.ConfigDict:
...
class TronWfJoystickEnv(base.TronWfBaseEnv):
    def __init__(
            self,
            task: str = None,
            config: config_dict.ConfigDict = tron_wf_joystick_config(),
    ...
    def _post_init(self):
        ...
        for side in side_names:
            for joint_name in wheel_joint_names:
                wheel_indices.append(
                    self._mj_model.joint(f"{joint_name}_{side}_Joint").qposadr - 7
                )
        self._wheel_indices = jp.array(wheel_indices)

        self._vel_indices = self._wheel_indices
        self._pos_indices = jp.array(
            [i for i in range(self._mj_model.nu) if i not in self._vel_indices]
        )
